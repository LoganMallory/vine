double benchmark_equal_to_constant() {
  Vine<int> vec1(SIZE);
  ...
  for(unsigned int i = 0; i < ITERATIONS; i++) {
    ...
    v = MIN + (rand() % static_cast<int>(MAX - MIN + 1));

    //start benchmark
    std::chrono::time_point< std::chrono::system_clock > start = std::chrono::system_clock::now();

    //do work
    Vine<bool> vec2 = vec1 == v;
...
double benchmark_equal_to_vine() {
  Vine<int> vec1(SIZE);
  Vine<int> vec2(SIZE);
...
double benchmark_not_equal_to_constant() {
  Vine<int> vec1(SIZE);
...
double benchmark_not_equal_to_vine() {
  Vine<int> vec1(SIZE);
  Vine<int> vec2(SIZE);
